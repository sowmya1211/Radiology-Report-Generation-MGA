...
def main():
    ...
    """Gets dataframe of train and validation sets 
        with colums "mimic_image_file_path", "bbox_coordinates", "bbox_labels"
        no of samples to be used is reduced based on PERCENTAGE_OF_TRAIN_SET_TO_USE, PERCENTAGE_OF_VAL_SET_TO_USE"""
    datasets_as_dfs = get_datasets_as_dfs(config_file_path) 

    """Gets transformations to be applied to train and test set respectively"""
    train_transforms = get_transforms("train")
    val_transforms = get_transforms("val")

    """Applies corresponding transforms to the elements of the dataframes.
        Returns sample = {
    ...
    """Loads data in batches and uses collate fn to form minibatch"""
    train_loader, val_loader = get_data_loaders(train_dataset, val_dataset)

    """Loads the object detector module
        setting return_feature_vectors = False  means the model is being trained or evaluated 
            in isolation and not as a part of the whole model"""
